...
- `/generate-discount` (admin) creates a Stripe promotion code for the Botivo checkout coupon box.
...
3. Copy `.env.example` → `.env` and fill `SUPPORT_ROLE_ID`, `TICKET_LOG_CHANNEL_ID`, and optionally `TICKET_CATEGORY_ID`, `PANEL_CHANNEL_ID`, `WEBSITE_URL`, and `STRIPE_SECRET_KEY` for `/generate-discount`.
4. `npm install && npm start` (from this `ticket-bot/` folder).

## Railway

No second service needed: on the existing Vexel Store service add `TICKET_BOT_TOKEN` (this bot's token) plus `SUPPORT_ROLE_ID`, `TICKET_LOG_CHANNEL_ID`, and optionally `TICKET_CATEGORY_ID` / `PANEL_CHANNEL_ID` / `WEBSITE_URL`. Both bots then run in one process. (To run only this bot, set `BOT_MODE=ticket`.)
